...
public interface EstudanteRepository extends JpaRepository<EstudanteModel, Long>{
    ...
    @Query(value = "SELECT e.id, e.nome_estudante, e.idade, c.nome_curso, c.descricao, c.carga_horaria"+
                    "FROM Estudantes e"+ 
                    "INNER JOIN Cursos c"+ 
                    "ON e.curso = c.id", nativeQuery = true)
    Optional<EstudanteCursoDTO> findAllEstudantesWithCurso();
}
